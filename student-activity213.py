...
def number_of_vehicle():
    ...
    if len(all_vehicles) > 1:
        ...
        for distance_of_car, vehicles in sorted_vehicles: #complete this code
            if distance_f_car > 200.0: #complete this code
                break
...
def car_control():
    ...
    time.sleep(3)
    dropped_vehicle.apply_control(carla.VehicleControl(hand_brake=True))
    dropped_vehicle.set_light_state(carla.VehicleLightState(carla.VehicleLightState.BrakeLight)) #Turn on Brake light
    time.sleep(5)
try:
